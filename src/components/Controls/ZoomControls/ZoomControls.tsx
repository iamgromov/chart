...
import IconReload from '../../../assets/icons/icon-reload.svg?react';
...
interface ZoomControlsProps {
  ...
  onZoomChange: (_domain: [Date, Date] | null) => void;
  onResetZoom: () => void;
}
...
export function ZoomControls({
  ...
  const handleZoomOut = useCallback(() => {
    ...
    if (
      clampedStart.getTime() === fullStart.getTime() &&
  ...
  return (
    <div className={styles['zoom-controls']}>
      <div className={styles['zoom-controls__group']}>
        <Button onClick={handleZoomOut} disabled={!isZoomed} title='Zoom out' aria-label='Zoom out'>
          <IconMinus />
        </Button>

        <Button onClick={handleZoomIn} disabled={!canZoomIn()} title='Zoom in' aria-label='Zoom in'>
          <IconPlus />
        </Button>
      </div>
